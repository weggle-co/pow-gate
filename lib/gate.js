...
/**
 * A store may be backed by a network call, in which case `add` returns a
 * promise. A promise is always truthy, so treating one as a boolean answer
 * would report every replay as a fresh spend.
 */
...
function createPowGate(options = {}) {
   ...
   * they had not solved.
   *
   * @returns {boolean|Promise<boolean>} false if this token had already been
   *   spent. A store whose `add` is asynchronous makes this a promise.
   */
   ...
   *
   * Returns the same shape the store works in. A synchronous store — the
   * built-in one, or any plain object — gives a plain result. A shared store
   * backed by Redis returns a promise from `add`, so this returns a promise
   * too, and the caller must await it. Awaiting is safe either way, so a
   * handler that always awaits works with both.
   */
  function verifyAndConsume(token, nonce, opts = {}) {
    const result = verify(token, nonce, opts);
    if (!result.ok) return result;

    const spent = consume(token);
    const settle = (ok) => (ok ? result : { ok: false, reason: 'already used' });

    // Not `await`, because that would make the synchronous case asynchronous
    // as well, and a caller who never awaits would silently stop seeing
    // replays — which is the failure this whole function exists to prevent.
    return isThenable(spent) ? spent.then(settle) : settle(spent);
  }
